...
public class ProgressBarUi extends BasicProgressBarUI {
	...
	//	private static LocalDateTime lastInstance;
	...
	public ProgressBarUi() {
		try {
			bimage = ImageIO.read( this.getClass().getResource( "/background2.png" ) );
		}
		catch (IOException e) {
			e.printStackTrace();
		}

//		if ( lastInstance == null || lastInstance.isBefore( LocalDateTime.now().minusSeconds( 5 ) ) ) {
//			lastInstance = LocalDateTime.now();
		Random rnd = new Random();
		character = Character.values()[rnd.nextInt( 0, Character.values().length )];
//		}
	}
	...
	protected void paintIndeterminate(Graphics g2d, JComponent c) {
		...
		character.getIcon().paintIcon( progressBar, g, offset2 - JBUIScale.scale( 3 ), -JBUIScale.scale( -2 ) );
	...
	protected void paintDeterminate(Graphics g, JComponent c) {
		...
		if ( progressBar.getOrientation() != SwingConstants.HORIZONTAL || !c.getComponentOrientation()
				.isLeftToRight() ) {
		...
		if ( bimage != null ) {
			TexturePaint tp = new TexturePaint(
					bimage,
					new Rectangle2D.Double( 0, 1, 2 * ( h - 2f * off - off ), h - 2f * off - off )
			);
			g2.setPaint( tp );
		}

		// fill progress with background texture
		...
		// draw lok
		character.getIcon().paintIcon( progressBar, g2, amountFull - JBUIScale.scale( 5 ), -JBUIScale.scale( 1 ) );
